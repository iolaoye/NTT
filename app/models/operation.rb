class Operation < ActiveRecord::Base
  attr_accessible :amount, :crop_id, :day, :depth, :month_id, :nh3, :no3_n, :activity_id, :org_n, :org_p, :po4_p, :type_id, :year, :subtype_id, :moisture, :scenario_id, :org_c, :nh4_n, :rotation
  #associations
  has_many :crops
  has_many :activities
  ...
  def sum
    if self.activity_id == 2 || self.activity_id == 7 
	  if !((self.no3_n.to_f + self.org_n.to_f) <= 100 || (self.po4_p.to_f + self.org_p.to_f) <= 100)
