...
################################  run_scenario - run simulation called from show or index  #################################
  def run_scenario()
    ...
    if msg.eql?("OK")
      msg = create_site_file(@scenario.field_id)
    end
    if msg.eql?("OK")
      msg = create_weather_file(dir_name, @scenario.field_id)
    end
    if msg.eql?("OK")
    ...
    end #this operation will run a simulation and return ntt file.
    session[:depth] = ""
    if msg.include?("NTT OUTPUT INFORMATION")
