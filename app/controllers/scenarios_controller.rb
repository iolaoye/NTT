...
#	end
	...
	def read_apex_results(number_of_soils, yearRotation , chk_cb_selected)
        Dim tempa As String
        ...
        ntt_apex_results = load_results(apex_start_year)
        #inicialize results and average all of the totals but crops
        AverageTotals(ntt_apex_results)
        'take results from .ACY for crops into _cropsInfo
        LoadCropResults(_cropsInfo, apex_start_year)
        ...
        CalculateCITotal(ntt_apex_results)
        CalculateCIbySoil(ntt_apex_results, _cropsInfo)
        If ddlType.SelectedIndex = 0 Then
            LoadMonthValues(apex_start_year, _fieldsInfo1(currentFieldNumber)._scenariosInfo(currentScenarioNumber)._results)
        Else
            LoadMonthValues(apex_start_year, _subprojectName(ddlSubproject.SelectedIndex)._results)
        End If


        LoadAnnualValues(ntt_apex_results, nAvgyears, _cropsInfo)
